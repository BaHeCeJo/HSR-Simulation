...
async function installPackages(pkgManager, packages) {
	if (!packages.length) {
		log(c.green("✁E), c.bold("All packages are already installed."));
		return;
	}
...
async function updatePkgJsonScripts(pkgJsonPath, vitestScript) {
	...
	log(c.green("✁E), "Added \"test:browser\" script to your package.json.");
}
...
async function create() {
	...
	if (rootConfig) {
		...
		log(c.green("✁E), "Created a config file for browser tests:", c.bold(relative(process.cwd(), configPath)));
	}
	log();
	...
	log(c.green("✁E), "Created example test file in", c.bold(relative(process.cwd(), exampleTestFile)));
