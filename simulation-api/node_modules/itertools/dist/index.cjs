"use strict";Object.defineProperty(exports, "__esModule", {value: true}); function _nullishCoalesce(lhs, rhsFn) { if (lhs != null) { return lhs; } else { return rhsFn(); } }// src/utils.ts
...
var groupby = igroupby;
function* igroupby(iterable, keyFn = primitiveIdentity) {
  const it = iter(iterable);
  let currentValue;
...
function indexBy(iterable, keyFn) {
  const result = {};
  for (const item of iterable) {
    const key = keyFn(item);
    result[key] = item;
...
function first(iterable, keyFn) {
  return find(iterable, _nullishCoalesce(keyFn, () => ( isDefined)));
}
