...
test('redact option  Ethrows if not array', async () => {
  assert.throws(() => {
    pino({ redact: 'req.headers.cookie' })
  })
})

test('redact option  Ethrows if array does not only contain strings', async () => {
  assert.throws(() => {
    pino({ redact: ['req.headers.cookie', {}] })
  })
})

test('redact option  Ethrows if array contains an invalid path', async () => {
  assert.throws(() => {
    pino({ redact: ['req,headers.cookie'] })
  })
})

test('redact.paths option  Ethrows if not array', async () => {
  assert.throws(() => {
    pino({ redact: { paths: 'req.headers.cookie' } })
  })
})

test('redact.paths option  Ethrows if array does not only contain strings', async () => {
  assert.throws(() => {
    pino({ redact: { paths: ['req.headers.cookie', {}] } })
  })
})

test('redact.paths option  Ethrows if array contains an invalid path', async () => {
  assert.throws(() => {
    pino({ redact: { paths: ['req,headers.cookie'] } })
  })
})

test('redact option  Etop level key', async () => {
  const stream = sink()
...
test('redact option  Etop level key next level key', async () => {
  const stream = sink()
...
test('redact option  Enext level key then top level key', async () => {
  const stream = sink()
...
test('redact option  Eobject', async () => {
  const stream = sink()
...
test('redact option  Echild object', async () => {
  const stream = sink()
  const instance = pino({ redact: ['req.headers.cookie'] }, stream)
  instance.child({
    req: {
...
test('redact option  Einterpolated object', async () => {
  const stream = sink()
  const instance = pino({ redact: ['req.headers.cookie'] }, stream)
...
test('redact.paths option  Eobject', async () => {
  const stream = sink()
...
test('redact.paths option  Echild object', async () => {
  const stream = sink()
  const instance = pino({ redact: { paths: ['req.headers.cookie'] } }, stream)
  instance.child({
    req: {
...
test('redact.paths option  Einterpolated object', async () => {
  const stream = sink()
  const instance = pino({ redact: { paths: ['req.headers.cookie'] } }, stream)
...
test('redact.censor option  Esets the redact value', async () => {
  const stream = sink()
...
test('redact.censor option  Ecan be a function that accepts value and path arguments', async () => {
  const stream = sink()
...
test('redact.censor option  Ecan be a function that accepts value and path arguments (nested path)', async () => {
  const stream = sink()
...
test('redact.remove option  Eremoves both key and value', async () => {
  const stream = sink()
...
test('redact.remove  Etop level key - object value', async () => {
  const stream = sink()
...
test('redact.remove  Etop level key - number value', async () => {
  const stream = sink()
  const instance = pino({ redact: { paths: ['key'], remove: true } }, stream)
  instance.info({
    key: 1
...
test('redact.remove  Etop level key - boolean value', async () => {
  const stream = sink()
  const instance = pino({ redact: { paths: ['key'], remove: true } }, stream)
  instance.info({
    key: false
...
test('redact.remove  Etop level key in child logger', async () => {
  const stream = sink()
...
test('redact  Esupports last position wildcard paths', async () => {
  const stream = sink()
...
test('redact  Esupports first position wildcard paths', async () => {
  const stream = sink()
...
test('redact  Esupports first position wildcards before other paths', async () => {
  const stream = sink()
...
test('redact  Esupports first position wildcards after other paths', async () => {
  const stream = sink()
...
test('redact  Esupports first position wildcards after top level keys', async () => {
  const stream = sink()
...
test('redact  Esupports top level wildcard', async () => {
  const stream = sink()
...
test('redact  Esupports top level wildcard with a censor function', async () => {
  const stream = sink()
  const instance = pino({
    redact: {
      paths: ['*'],
...
test('redact  Esupports top level wildcard and leading wildcard', async () => {
  const stream = sink()
...
test('redact  Esupports intermediate wildcard paths', async () => {
  const stream = sink()
