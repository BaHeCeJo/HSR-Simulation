 ...
 * ```
 * username
 *   ✁EExpected number, received string at "username
 * favoriteNumbers[0]
 *   ✁EInvalid input: expected number
 * ```
 */
...
export function prettifyError(error: StandardSchemaV1.FailureResult): string {
  ...
  for (const issue of issues) {
    lines.push(`✁E${issue.message}`);
    if (issue.path?.length) lines.push(`  ↁEat ${toDotPath(issue.path)}`);
  }
