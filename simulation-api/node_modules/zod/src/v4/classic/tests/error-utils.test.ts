...
test("z.prettifyError", () => {
  expect(z.prettifyError(result.error!)).toMatchInlineSnapshot(`
    "✁EUnrecognized key: "extra"
    ✁EInvalid input: expected string, received number
      ↁEat username
    ✁EInvalid input: expected number, received string
      ↁEat favoriteNumbers[1]
    ✁EInvalid input: expected string, received number
      ↁEat nesting.a"
  `);
});
