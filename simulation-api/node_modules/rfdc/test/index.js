...
test('default  Edoes not copy proto properties', async ({ is }) => {
  is(clone(Object.create({ a: 1 })).a, undefined, 'value not copied')
})
test('default  Eshorthand import', async ({ same }) => {
  same(
...
test('proto option  Ecopies enumerable proto properties', async ({ is }) => {
...
test('circles option  Edeep circular object', async ({ same, is, isNot }) => {
  const o = { nest: { a: 1, b: 2 } }
  o.nest.circular = o
  same(cloneCircles(o), o, 'same values')
  isNot(cloneCircles(o), o, 'different objects')
...
test('circles option alone  Edoes not copy proto properties', async ({
  is
}) => {
  is(cloneCircles(Object.create({ a: 1 })).a, undefined, 'value not copied')
})
test('circles and proto option  Ecopies enumerable proto properties', async ({
  is
}) => {
...
test('circles and proto option  Edeep circular object', async ({
  same,
  is,
  isNot
}) => {
...
test('circles and proto option  Edeep circular array', async ({
  same,
  is,
  isNot
}) => {
...
function types (clone, label) {
  test(label + '  Enumber', async ({ is }) => {
    is(clone(42), 42, 'same value')
  })
  test(label + '  Estring', async ({ is }) => {
    is(clone('str'), 'str', 'same value')
  })
  test(label + '  Eboolean', async ({ is }) => {
    is(clone(true), true, 'same value')
  })
  test(label + '  Efunction', async ({ is }) => {
    const fn = () => {}
    is(clone(fn), fn, 'same function')
  })
  test(label + '  Easync function', async ({ is }) => {
    const fn = async () => {}
    is(clone(fn), fn, 'same function')
  })
  test(label + '  Egenerator function', async ({ is }) => {
    const fn = function * () {}
    is(clone(fn), fn, 'same function')
  })
  test(label + '  Edate', async ({ is, isNot }) => {
    const date = new Date()
    is(+clone(date), +date, 'same value')
    isNot(clone(date), date, 'different object')
  })
  test(label + '  Enull', async ({ is }) => {
    is(clone(null), null, 'same value')
  })
  test(label + '  Eshallow object', async ({ same, isNot }) => {
    const o = { a: 1, b: 2 }
    same(clone(o), o, 'same values')
    isNot(clone(o), o, 'different object')
  })
  test(label + '  Eshallow array', async ({ same, isNot }) => {
    const o = [1, 2]
    same(clone(o), o, 'same values')
    isNot(clone(o), o, 'different arrays')
  })
  test(label + '  Edeep object', async ({ same, isNot }) => {
    const o = { nest: { a: 1, b: 2 } }
    same(clone(o), o, 'same values')
    isNot(clone(o), o, 'different objects')
    isNot(clone(o).nest, o.nest, 'different nested objects')
  })
  test(label + '  Edeep array', async ({ same, isNot }) => {
    const o = [{ a: 1, b: 2 }, [3]]
    same(clone(o), o, 'same values')
    isNot(clone(o), o, 'different arrays')
  ...
  test(label + '  Enested number', async ({ is }) => {
    is(clone({ a: 1 }).a, 1, 'same value')
  })
  test(label + '  Enested string', async ({ is }) => {
    is(clone({ s: 'str' }).s, 'str', 'same value')
  })
  test(label + '  Enested boolean', async ({ is }) => {
    is(clone({ b: true }).b, true, 'same value')
  })
  test(label + '  Enested function', async ({ is }) => {
    const fn = () => {}
    is(clone({ fn }).fn, fn, 'same function')
  })
  test(label + '  Enested async function', async ({ is }) => {
    const fn = async () => {}
    is(clone({ fn }).fn, fn, 'same function')
  })
  test(label + '  Enested generator function', async ({ is }) => {
    const fn = function * () {}
    is(clone({ fn }).fn, fn, 'same function')
  })
  test(label + '  Enested date', async ({ is, isNot }) => {
    const date = new Date()
    is(+clone({ d: date }).d, +date, 'same value')
    isNot(clone({ d: date }).d, date, 'different object')
  })
  test(label + '  Enested date in array', async ({ is, isNot }) => {
    const date = new Date()
  ...
  test(label + '  Enested null', async ({ is }) => {
    is(clone({ n: null }).n, null, 'same value')
  })
  test(label + '  Earguments', async ({ isNot, same }) => {
    function fn (...args) {
